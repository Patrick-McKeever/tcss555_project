# Linear Regression with SVD for neurotism
# date: 10/12/2024
# name: Terri Bell
...
data_Facebook = df.loc[:,['transcript', 'neu']]
...
y_train = data_Facebook['neu']
svd = TruncatedSVD(n_components=100)
X_train_svd = svd.fit_transform(X_train)
linreg = LinearRegression()
linreg.fit(X_train_svd, y_train)

model_path = os.path.join('models', 'lin_reg_svd_neu.pkl')
